(() => {
  ...
  document.querySelectorAll("[data-professor-code-refresh]").forEach(button=>button.addEventListener("click",async()=>{button.disabled=true;button.textContent="Generating…";const result=await db.rpc("regenerate_professor_access_code",{requested_section:button.dataset.professorCodeRefresh});field("staffReview").innerHTML=`<p class="${result.error?"auth-message":"form-message"}">${esc(result.error?`Professor code was not changed: ${result.error.message}`:`Professor code regenerated. The previous code no longer works. New code: ${result.data}`)}</p>`;if(!result.error)setTimeout(staffScreen,2500);else{button.disabled=false;button.textContent="Regenerate professor code";}}));
  ...
  function bindOperationButtons(){
    ...
    document.querySelectorAll("[data-section-code-refresh]").forEach(button=>button.addEventListener("click",async()=>{button.disabled=true;button.textContent="Generating…";const result=await db.rpc("regenerate_section_code",{requested_section:button.dataset.sectionCodeRefresh});if(result.error){showOperationResult(`Student section code was not changed: ${result.error.message}`,true);button.disabled=false;button.textContent="Regenerate student section code";}else{showOperationResult(`Student section code regenerated. The previous code no longer works. New code: ${result.data}`);setTimeout(staffScreen,2500);}}));
    ...
    document.querySelectorAll("[data-team-control]").forEach(button=>button.addEventListener("click",async()=>{if(button.dataset.teamControl==="code"){button.disabled=true;button.textContent="Generating…";const result=await db.rpc("regenerate_team_code",{requested_team:button.dataset.teamId});if(result.error){showOperationResult(`Team code was not changed: ${result.error.message}`,true);button.disabled=false;button.textContent="Regenerate team code";}else{showOperationResult(`Team code regenerated. The previous code no longer works. New code: ${result.data}`);setTimeout(staffScreen,2500);}return;}const changes={};if(button.dataset.teamControl==="lock")changes.mission_locked=button.dataset.locked!=="true";if(button.dataset.teamControl==="mission")changes.active_mission=Number(document.querySelector(`[data-mission-select="${button.dataset.teamId}"]`)?.value);const result=await db.from("teams").update(changes).eq("id",button.dataset.teamId);if(result.error)showOperationResult(result.error.message,true);else staffScreen();}));
  ...
  return `<div class="team-operations">${teams.map(team=>{
    ...
    return `<article class="team-operation">
      ...
      <div class="team-code-row"><code>${esc(team.join_code)}</code><button class="btn compact" type="button" data-copy-code="${esc(team.join_code)}">Copy team code</button><button class="btn compact" type="button" data-team-control="code" data-team-id="${team.id}">Regenerate team code</button></div>
