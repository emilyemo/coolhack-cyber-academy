(() => {
  ...
  const missionCatalog = [
    null,
    {title:"The Urgent Invoice", subtitle:"Phishing triage", scenario:"A finance employee received an urgent invoice email that appears to come from a trusted vendor. The employee opened the message and clicked the link before reporting it. The team must determine whether the message is malicious and what may have been exposed."},
    {title:"Impossible Travel", subtitle:"Identity investigation", scenario:"The monitoring system reports successful logins for one employee from two distant locations within an impossible time window. One login used a familiar device; the other did not. The team must decide whether this is credential theft, legitimate travel, or a false positive."},
    {title:"The Noisy Workstation", subtitle:"Endpoint malware", scenario:"A workstation begins making repeated outbound connections shortly after a free utility was installed. The user says the computer still works normally. The team must decide whether to isolate it without unnecessarily disrupting operations."},
    {title:"After-Hours Database Query", subtitle:"Insider-risk analysis", scenario:"A staff member with legitimate database access downloaded an unusually large set of customer records after normal business hours. Authorization alone does not explain intent. The team must investigate without assuming guilt."},
    {title:"Website Under Pressure", subtitle:"Availability incident", scenario:"The public website has become slow and intermittently unavailable. Traffic volume has spiked, but marketing also launched a campaign today. The team must determine whether this is a denial-of-service attack, legitimate demand, or both."},
    {title:"Executive Incident Briefing", subtitle:"Capstone case", scenario:"Leadership needs a final briefing on a multi-stage incident involving phishing, compromised credentials, suspicious endpoint activity, and possible data access. The team must combine the technical findings into one defensible story."}
  ];
